...
export interface DemoState { schemaVersion: 1; locale: Locale; scenario: DemoScenario; currentResidentId: string; currentBuildingId: string; buildings: Building[]; units: Unit[]; residents: Resident[]; invoices: Invoice[]; payments: Payment[]; serviceFamilies: ServiceFamily[]; serviceOfferings: ServiceOffering[]; providers: ServiceProvider[]; orders: ServiceOrder[]; memberOffers: MemberOffer[]; recurringPlans: RecurringPlan[]; neighborDeals: NeighborDeal[]; neighborRelationships: NeighborRelationship[]; announcements: Announcement[]; polls: Poll[]; events: CommunityEvent[]; visitorPasses: VisitorPass[]; amenityBookings: AmenityBooking[]; gifts: NeighborGift[]; activities: Activity[]; auditLog: AuditEntry[]; }
